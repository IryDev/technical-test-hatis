...
export const Table = () => {
  ...
  const loadCells = async () => {
    try {
      const response = await fetch("http://localhost:5500/api/table");
  ...
  const handleEdit = async (id: string, newValue: string) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5500/api/table/cells/${id}`,
        {
          method: "POST",
          headers: {
  ...
  const handleReset = async () => {
    try {
      const response = await fetch("http://localhost:5500/api/table/reset", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
